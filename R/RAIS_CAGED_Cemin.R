  ...
  if (!length(resyr) & !length(prevres)) {
    ...
    if (!length(resuf) & !length(prevres)) {
      ...
      if (!length(res)) {
        ...
        # seleção de subclasses 2.0 _________________________________________________________________________________
         
        if (any(res=='CNAE 2.0 Subclasse') | !length(res)) {
          dlgMessage(c('Deseja salvar os dados de apenas algumas Subclasses CNAE 2.0?',
                       'digite a seguir, sem o zero na frente (6 ou 7 dígitos)'))
          ressubCNAE <- dlgInput(c('Digite cada código separado por vírgula, como no',
        ...
        if (!length(filesres)) {
          ...
          for(i in filest){
            ...
            # seleção de subclasses 2.0 _________________________________________________
